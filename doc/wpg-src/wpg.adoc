...
On MS-Windows, the recommended approach is to install WSL (Windows Subsystem
for Linux).  WSL provides a full GNU/Linux environment, including `gcc` and
all other build tools, and the WFDB Software Package can be built and used
exactly as on any other Linux system.  To install WSL, run `wsl --install`
...
WSL (Windows Subsystem for Linux)::
  ...
  To install, run `wsl --install` in an administrator PowerShell or
  Command Prompt.  WSL provides a full GNU/Linux environment (typically
  Ubuntu) on MS-Windows, including `gcc` and all standard development
  tools.  The WFDB Software Package can be built and used under WSL
  exactly as on any other Linux system.  This is the recommended way to
  use WFDB on MS-Windows.
...
internal state—open signals and annotations, the WFDB path, calibration
data, time conversion parameters, and so on—so that multiple independent
