...
have a shell utility such as `bash` (available via WSL on MS-Windows) in
order to run `ahaconvert`.

### Obsolete MIT Header Format
...
the output generated from the input to wfdb@physionet.org. (Don't omit the
